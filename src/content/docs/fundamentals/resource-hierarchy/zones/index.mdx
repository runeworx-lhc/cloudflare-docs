...
A _domain_ or _domain name_ is the location of a website or application, or what an end user types into their browser to get to your website (`example.com`). At Cloudflare, domain names get bound into what we call **Zones**. Zones are one of the main ways we group settings throughout the product for features that are related to your network configuration.
